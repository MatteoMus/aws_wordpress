...
The application is deployed on a 3 Tiers and 2 AZs infrastructure. The DB, EC2 (with the webserver) and EFS belong on private subnets. In the public subnets reside the ALB and NAT GATEAYs. An external S3 bucket is used by Beanstalk to retrieve app informations for the deploy. Secret manager is used to store the DB password. If you have a domain in Route53 with a hosted zone, you could put in front of your ALB. The DB is external to Beanstlk container, this way you can choose Aurora as engine and you can destroy and rebuilt the Beanstalk container without lose your data.
...
Both DB and instances are deployed into private subnets. The DB password is automatically generated with cloufromation and saved in a Secret Manager, that is retrieved by the instances at boot. The communication between internet and ALB is based on HTTPS protocol. The communication EC2-EFS and EC2-DB are secured by security groups. EC2s and DB reach internet through the NAT Gateways, so there is now way that a communication with them is started from internet. The SSH port of the instances is closed, only with a System Manager Session we can login into the webservers. Any updates of the EC2s can be managed with System Manager.

### 3.2 - Fast

The ALB lets the application scales horizontally and you can choose the EC2s'size that better fits your needs. The DB is a mysql Amazon Aurora Serverless for better performance than a classic mysql. It uses Secret Manager to store the password since secure and faster then IAM.

### 3.3 - Fault tolerant

The use of an ALB with an Autoscaling Group covering 2 AZs ensures fault tollerance. The EC2 share a EFS to store the uploaded files through the web application interface. The DB instances are serverless (so managed by AWS) and deployed on different AZs.
...
Move to cftemplate folder. Inside there is a **root.yaml** and folder **nested**. Upload the yaml files inside the folder **nested** into a S3 bucket in an account where you are able to run cloudformation. The yaml files must reside at the root path of your bucket. In your local PC configure the AWS CLI to access this account.
...
Parameters required:
- S3CFNAME = name of the bucket where you saved the cloudformation files
...
- SSLCertificateArns = arn of a certificate saved in AWS Certificate Manager
...
- HostedZoneId = Hosted zone ID. You will find it in your hosted zone details
...
>You can deploy any wordpress version that supports PHP 8.1.
...
In the root path there is a source_bunde.zip ready to be deployed to test the environment.
...
This is the wordpress configuration file. It's modified to retrieve the DB credentials and informations from the file /var/app/current/secret.json
